...
head(xts_table,5)
### save(xts_table,file="D://xts_table.Rdata") #保存Rdata类型数据
### load("D://xts_table.Rdata")  #自本地保存的rdata读取数据
...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    rolling_sharpe_xts_p <-
      rollapply(portfolio_return_daily,
                input$window,
                function(x)
                  SharpeRatio(x,
                              Rf = input$rfr,
...
ui <- fluidPage(
  ...
  titlePanel("Rolling Sharpe Ratio Calculation"),
  
  sidebarLayout(
    sidebarPanel(
      numericInput(inputId = "rfr",
                   label = "无风险收益率",
                   min= 0,
                   max=0.1,
                   value = 0.03),
